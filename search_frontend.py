...
#tokenize function for a single query(string
def tokenize(text):
    tokens = [token.group() for token in RE_WORD.finditer(text.lower())]
    return tokens
#tokenize function for a list of queries(strings)
...
def word_count_anchor(id,list_of_tupels):
  ''' Count the frequency of each word in `text` (anchor text) that is not included in 
  `all_stopwords` and return entries that will go into our posting lists for the anchor index. 
  Parameters:
  -----------
    text: str
      Text of one document
...
#Loads Document Length files for the title index and body index
import json
...
#Function returns specific posting lists for the terms provided in the given index
...
#function returns all the doc_ids in the posting lists of the qiven query terms in the provided index
...
#uses the ranking function above to get the relevent documents to return and gets the titles of those documents 
...
def getPr(id):
  ...
  for t in pr_k:
    if t[0]==id:
      return t[1]
  return 0
#Main page rank function. returns the page rank from getPr function for each document in the id_list.
...
#get the page views of a single document from the json files on the machine disk
def getPv(id):
  k=id%800
  ...
  for t in pr_k:
    if t[0]==id:
      return t[1]
  return 0
#Main page views function. returns the page rank from getPv function for each document in the id_list.
...
def get_candidate_documents(query_to_search,index,pls):
    ...
    pls: relevent postings lists for query terms
    Returns:
    -----------
...
#function returns the title of the given document id from the stored json files.
def getTitle(id):
  k=id%800
...
#Main search function used in the frontend.
#Utilizing the best weights we found for the BM-25 scores merge funciton.
...
#Test function for for returing bm-25 socres for the training set of queries
...
#weight test function for the merging of bm-25 scores
def test_diff_weights_4merge_two(bm25_body,bm25_title,increments = 5):
 ...
 #Importing of the training queries and weight test function call               
import json
if False:
